...
- Multi-map reinsertion Phase B (2026-09-09): first in-game travel round-trip WORKS on 1.38 — vanilla->map_test fresh + map_start applied (`[GNX-MAP] map_start applied for 'map_test' (floors+=1 cells=2)`), T54-57 PASS, 46/46 suite. Fixes this iteration: (1) return marker now defaults to the outbound marker's x/y (`_tl.return_x = _mb.return_x ?? _tl.x`) instead of hardcoded (200,80) — it was landing under a stage label on the mod's map. (2) `global.gnx_active_map`/`gnx_inactive_maps` were persisted forever via `variable_global_exists` guards, so quit-to-menu -> New Game kept you on the last map. Now scr_create_initials resets them to vanilla/{} UNLESS `global.gnx_in_travel` is true (obj_control_Step_0 deferred-load block sets it true around its scr_create_initials() re-call, false after). KNOWN GAP for Phase C: loading a save that was made on a non-vanilla map — scr_gnx_load_mods runs inside scr_create_initials (active_map reset to vanilla) BEFORE scr_load_slot restores gnx_active_map from _load_data (~s_save_load L840), so that map's mods get skipped. Needs: read the saved active_map before mod loading (peek the save file early, or a two-pass). Travel-from-vanilla (fresh each session) is unaffected and works. <!-- id:c3ed -->
